...
module.exports.register = validate([
  ...
  .isLength({ min: 3 }).withMessage('用户名不能小于三位').bail(),
  body('email').notEmpty().withMessage('邮箱不能为空').bail()
    .isEmail().withMessage('邮箱格式不正确').bail()
    .isLength({ min: 3 }).withMessage('邮箱不能小于三位').bail()
    .custom(async val => {
  ...
  body('phone')
    ...
    .isLength({ min: 3 }).withMessage('手机号不能少于11位').bail()
    .custom(async val => {
  ...
  body('password')
    ...
    .isLength({ min: 6 }).withMessage('密码不能少于6位').bail()
])
